...
It generalizes the `hotload_server.py` prototype in the parent directory into a schema-driven, multi-variable, hybrid engine.
...
Then press F5 in VS Code/Positron to launch an Extension Development Host, open a `.qmd` (e.g. the `index.qmd` in the parent dir), and run **Quarto: Open Theme Tweaker** from the command palette.
